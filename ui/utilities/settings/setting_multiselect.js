...
class MultiselectSetting {
    ...
    /**
     * Creates a popup dialog for editing the multiselect setting.
     */
    createPopup() {
        this.popup = $('.FU-settings-screen').createPopupDialog('Change Multiselect', this.data.name, null, 'change-array-popup');
...
var ArraySetting = function(mod, page, setting, parentDiv) {
    console.warn("ArraySetting automatically transformed into MultiselectSetting.");
    return new MultiselectSetting(mod, page, setting, parentDiv);
};
...
let setting = {
    ...
    id: 'exampleMultiselectSetting',
    data: { lockLength: false },
    value: [['Item1', 'Value1'], ['Item2', 'Value2']]
};
let parentDiv = $('<div></div>'); // Example parent div

let multiselectSetting = new MultiselectSetting(mod, page, setting, parentDiv);
