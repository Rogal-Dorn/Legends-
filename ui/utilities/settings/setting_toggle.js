 ...
 * This class handles the creation, display, and interaction with a toggle-based setting.
 */
class ToggleSetting {
    /**
    ...
    createTitlebox(mod) {
        ...
        this.title = $(`<label class="toggle-checkbox-label title-font-normal font-color-title" for="${id}">${this.data.name}</label>`);
        this.title.click(() => {
...
var BooleanSetting = function(mod, page, setting, parentDiv) {
    console.warn("Boolean Setting automatically transformed to a Toggle Setting");
    return new ToggleSetting(mod, page, setting, parentDiv);
};
...
let setting = {
    ...
    id: 'exampleToggleSetting',
    value: true,
    locked: false
};
let parentDiv = $('<div></div>'); // Example parent div

let toggleSetting = new ToggleSetting(mod, page, setting, parentDiv);
