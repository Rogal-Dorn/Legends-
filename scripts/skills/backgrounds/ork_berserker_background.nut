this.ork_berserker_background <- this.inherit("scripts/skills/backgrounds/character_background", {
	m = {
		Tattoo = 0
	},
	function create()
	{
		...
		this.m.ID = "background.ork_berserker";
		this.m.Name = "Ork Berserker";
		this.m.Icon = "ui/backgrounds/background_ork_berserker.png";
		this.m.BackgroundDescription = "Bersekers are used to the hard life of the wild where only the strong prevail. They are less used to the life of cities, where the astute and deceitful rule.";
		...
		this.m.Faces = this.Const.Faces.OrkBerserker;
		this.m.Hairs = this.Const.Hair.WildMale;
		this.m.HairColors = this.Const.HairColors.Young;
		this.m.Beards = this.Const.Beards.Wild;
		this.m.BeardChance = 10;
		this.m.Bodies = this.Const.Bodies.OrkBerserker;
		this.m.Level = this.Math.rand(1, 3);
		this.m.BackgroundType = this.Const.BackgroundType.Greenskin |this.Const.BackgroundType.Combat | this.Const.BackgroundType.Outlaw | this.Const.BackgroundType.Druid | this.Const.BackgroundType.Ranger | this.Const.BackgroundType.ExpertHunter;
	...
	function onAddEquipment()
	{
		...
		local r;
			...
			if (r == 0)
			{
				items.equip(this.new("scripts/items/weapons/greenskins/legend_limb_lopper"));
			}
			else if (r == 1)
			{
				items.equip(this.new("scripts/items/weapons/greenskins/legend_man_mangler"));
			}
			else if (r == 2)
			{
			...
			{
				items.equip(this.new("scripts/items/weapons/greenskins/legend_skin_flayer"));
			}
			else if (r == 5)
			{
				items.equip(this.new("scripts/items/weapons/greenskins/legend_bough"));
			}
