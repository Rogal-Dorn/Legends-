this.legend_drink_beer_skill <- this.inherit("scripts/skills/skill", {
	m = {
		Item = null
	},
	function setItem( _i )
	...
	function onUse( _user, _targetTile )
	{
		...
		if (_user.getID() == user.getID())
		{
			...
			{
			user.getSkills().add(this.new("scripts/skills/effects/legend_beer_buzz_effect"));
			}

			if (!user.isHiddenToPlayer())
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(user) + " drinks beer");
			}
		...
		else
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + " gives beer to " + this.Const.UI.getColorizedEntityName(user));
			}
