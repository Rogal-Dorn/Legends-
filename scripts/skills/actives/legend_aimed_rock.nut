this.legend_aimed_rock <- this.inherit("scripts/skills/skill", {
	m = {
	...
	function create()
	{
		this.m.ID = "actives.legend_aimed_rock";
	...
	function getTooltip()
	{
		...
		if (ammo > 0)
		{
			ret.push({
				...
				text = "Has [color=" + this.Const.UI.Color.PositiveValue + "]" + ammo + "[/color] pebbles left"
			});
		}
		else
		{
	...
	function getAmmo()
	{
		...
		if (item.getAmmoType() == this.Const.Items.AmmoType.Pebbles)
		{
			return item.getAmmo();
		}
	}
	...
	function onAfterUpdate( _properties )
	{
		...
		this.m.AdditionalAccuracy = _properties.IsSpecializedInSlings ? -10 : -20;
