this.legend_drink_wine_skill <- this.inherit("scripts/skills/skill", {
	m = {
		Item = null
	},
	function setItem( _i )
	...
	function onUse( _user, _targetTile )
	{
		...
		if (_user.getID() == user.getID())
		{
			...
			{
			user.getSkills().add(this.new("scripts/skills/effects/legend_wine_tipsy_effect"));
			}

			if (!user.isHiddenToPlayer())
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(user) + " drinks wine");
			}
		...
		else
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + " gives wine to " + this.Const.UI.getColorizedEntityName(user));
			}
