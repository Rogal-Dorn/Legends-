this.legend_drink_skin_ghoul_blood_skill <- this.inherit("scripts/skills/skill", {
	m = {
		Item = null
	},
	function setItem( _i )
	...
	function onUse( _user, _targetTile )
	{
		...
		{
			user.getSkills().add(this.new("scripts/skills/effects/legend_skin_ghoul_blood_effect"));

			if (!user.isHiddenToPlayer())
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(user) + " drinks Skin Ghoul Blood");
			}
		...
		else
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + " gives Skin Ghoul Blood to " + this.Const.UI.getColorizedEntityName(user));
			}
