this.drink_cat_skill <- this.inherit("scripts/skills/skill", {
	m = {
		Item = null
	},
	function setItem( _i )
	...
	function onUse( _user, _targetTile )
	{
		...
		{
			user.getSkills().add(this.new("scripts/skills/effects/cat_potion_effect"));

			if (!user.isHiddenToPlayer())
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(user) + " drinks Cat Potion");
			}
		...
		else
		{
			...
			{
				this.Tactical.EventLog.log(this.Const.UI.getColorizedEntityName(_user) + " gives Cat Potion to " + this.Const.UI.getColorizedEntityName(user));
			}
