this.mod_unload_gifts_order <- this.inherit("scripts/ai/world/world_behavior", {
	m = {},
	function create()
	{
	...
	function onExecute( _entity, _hasChanged )
	{
		...
		this.World.FactionManager.getFactionOfType(this.Const.FactionType.Player).updateAlliancePlayerFaction(destination_faction);
		...
		if (!hasEvent)
		{
			local event = this.new("scripts/events/mod_stronghold/caravan_gifts_event")
