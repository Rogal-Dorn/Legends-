this.legend_ai_attack_prepare_graze <- this.inherit("scripts/ai/tactical/behavior", {
	m = {
		TargetTile = null,
		PossibleSkills = [
			"actives.legend_prepare_graze_skill"
		],
		Skill = null
	},
	function create()
	{
		this.m.ID = this.Const.AI.Behavior.ID.PrepareGraze;
		this.m.Order = this.Const.AI.Behavior.Order.PrepareGraze;
	...
	function onEvaluate( _entity )
	{
		...
		return this.Const.AI.Behavior.Score.PrepareGraze * score;
	}
	...
	function onExecute( _entity )
	{
		...
		if (this.m.TargetTile != null && this.m.TargetTile.IsOccupiedByActor)
		{
			...
			{
				this.logInfo("* " + _entity.getName() + ": Using graze against " + this.m.TargetTile.getEntity().getName() + "!");
			}
