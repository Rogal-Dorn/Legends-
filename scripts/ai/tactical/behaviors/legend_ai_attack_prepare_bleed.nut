this.legend_ai_attack_prepare_bleed <- this.inherit("scripts/ai/tactical/behavior", {
	m = {
		TargetTile = null,
		PossibleSkills = [
			"actives.legend_prepare_bleed_skill"
		],
		Skill = null
	},
	function create()
	{
		this.m.ID = this.Const.AI.Behavior.ID.PrepareBleed;
		this.m.Order = this.Const.AI.Behavior.Order.PrepareBleed;
	...
	function onEvaluate( _entity )
	{
		...
		return this.Const.AI.Behavior.Score.PrepareBleed * score;
	}
	...
	function onExecute( _entity )
	{
		...
		if (this.m.TargetTile != null && this.m.TargetTile.IsOccupiedByActor)
		{
			...
			{
				this.logInfo("* " + _entity.getName() + ": Using Exsanguinate against " + this.m.TargetTile.getEntity().getName() + "!");
			}
