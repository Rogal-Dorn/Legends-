	...
	function getBestTarget( _entity, _skill, _targets )
	{
		...
		foreach( target in _targets )
		{
			if (_skill.onVerifyTarget(_entity.getTile(), target.getTile()))
			{
				...
				local armor = target.getArmor(this.Const.BodyPart.Body) * p.getHitchance(this.Const.BodyPart.Body) * 0.00999999978 + target.getArmor(this.Const.BodyPart.Head) * p.getHitchance(this.Const.BodyPart.Head) * 0.00999999978;
