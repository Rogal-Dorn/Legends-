this.ghost_agent <- this.inherit("scripts/ai/tactical/agent", {
	...
	function onUpdate()
	{
		...
		if (!this.Tactical.State.isAutoRetreat() && !strategy.getStats().IsEngaged && this.m.Actor.getAttackedCount() <= 3 && this.Time.getRound() <= 2 && this.m.KnownAllies.len() >= 8 && ghosts < this.m.KnownAllies.len() - 1)
		{
