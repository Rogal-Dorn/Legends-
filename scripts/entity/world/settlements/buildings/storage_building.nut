this.storage_building <- this.inherit("scripts/entity/world/settlements/buildings/building", {
	...
	{
		//remove loot items if you sold them
		foreach( item in this.m.Stash.getItems() )
		{
			if (item != null)
			{
				item.setSold(false);
				if (item.m.ItemType == this.Const.Items.ItemType.Loot)
				{
