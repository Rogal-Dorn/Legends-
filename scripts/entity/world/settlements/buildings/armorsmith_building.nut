this.armorsmith_building <- this.inherit("scripts/entity/world/settlements/buildings/building", {
	...
	function onUpdateShopList()
	{
		...
		if (this.Const.DLC.Unhold)
		{
			...
			if (!this.World.LegendsMod.Configs().LegendArmorsEnabled()) //not exactly sure if this check needs to be here but for some reason armoursmiths are spawning regular shoulderguards with armor layers on so this at least fixes that, maybe it should be somewhere else?
			{
				list.push({
					R = 85,
					P = 1.25,
			...
			else
			{
				list.push({
					R = 85,
					P = 1.25,
					S = "armor_upgrades/legend_metal_plating_upgrade"
				});
				list.push({
					R = 85,
					P = 1.25,
					S = "armor_upgrades/legend_metal_pauldrons_upgrade"
				});
				list.push({
					R = 85,
					P = 1.25,
					S = "armor_upgrades/legend_mail_patch_upgrade"
				});
				list.push({
					R = 85,
					P = 1.25,
					S = "armor_upgrades/legend_leather_shoulderguards_upgrade"
				});
				list.push({
					R = 85,
					P = 1.25,
					S = "armor_upgrades/legend_leather_neckguard_upgrade"
				});
				list.push({
					R = 85,
					P = 1.25,
					S = "armor_upgrades/legend_joint_cover_upgrade"
				});
				list.push({
					R = 85,
					P = 1.25,
					S = "armor_upgrades/legend_heraldic_plates_upgrade"
				});
				list.push({
					R = 85,
					P = 1.25,
					S = "armor_upgrades/legend_double_mail_upgrade"
				});
			}
		}
