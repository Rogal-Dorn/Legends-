this.actor <- this.inherit("scripts/entity/tactical/entity", {
	...
	function isTurnDone()
	{
		...
		{
			return this.m.IsTurnDone || this.m.IsSkippingTurn || this.m.ActionPoints < this.Const.Movement.AutoEndTurnBelowAP && !this.m.Skills.isBusy();
		}
		else
		{
	...
	function onMovementInZoneOfControl( _entity, _isOnEnter )
	{
		...
		if (ValaTile != null)
		{
			if (ValaTile.getDistanceTo(this.getTile()) <= 2 && this.getSkills().hasSkill("effects.vazl_vala_chant_disharmony_effect"))
			{
				return false;
			}
		}
