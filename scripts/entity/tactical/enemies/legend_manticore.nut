this.legend_manticore <- this.inherit("scripts/entity/tactical/actor", {
	m = {
		Tail = null
	},
	...
	function create()
	{
		this.m.Type = this.Const.EntityType.LegendManticore;
		this.m.BloodType = this.Const.BloodType.Red;
		this.m.XP = this.Const.Tactical.Actor.LegendManticore.XP;
		...
		this.m.Sound[this.Const.Sound.ActorEvent.DamageReceived] = [
			"sounds/enemies/manticore_hit.wav"
		];
		this.m.Sound[this.Const.Sound.ActorEvent.Death] = [
			"sounds/enemies/manticore_die.wav"
		];
		...
		this.m.Sound[this.Const.Sound.ActorEvent.Idle] = [
			"sounds/enemies/manticore_idle.wav",
			"sounds/enemies/manticore_idle2.wav",
			"sounds/enemies/manticore_idle3.wav",
			"sounds/enemies/manticore_idle4.wav"
		];
		...
		this.getFlags().add("manticore");
		this.m.AIAgent = this.new("scripts/ai/tactical/agents/legend_manticore_agent");
	...
	function onDeath( _killer, _skill, _tile, _fatalityType )
	{
		if (_tile != null)
		{
			...
			decal = _tile.spawnDetail("bust_manticore_body_01_dead", this.Const.Tactical.DetailFlag.Corpse, flip);
			...
			{
				decal = _tile.spawnDetail("bust_manticore_head_01_dead", this.Const.Tactical.DetailFlag.Corpse, flip);
			...
			else if (_fatalityType == this.Const.FatalityType.Decapitated)
			{
				...
				local decap = this.Tactical.spawnHeadEffect(this.getTile(), layers, this.createVec(0, 0), 0.0, "bust_manticore_head_01_bloodpool");
			...
			{
				decal = _tile.spawnDetail("bust_manticore_body_01_dead_arrows", this.Const.Tactical.DetailFlag.Corpse, flip);
				decal.Scale = 0.95;
			}
			else if (_skill && _skill.getProjectileType() == this.Const.ProjectileType.Javelin)
			{
				decal = _tile.spawnDetail("bust_manticore_body_01_dead_javelin", this.Const.Tactical.DetailFlag.Corpse, flip);
				decal.Scale = 0.95;
			}
	...
	function onInit()
	{
		...
		b.setValues(this.Const.Tactical.Actor.LegendManticore);
		...
		body.setBrush("bust_manticore_body_0" + this.Math.rand(1, 1));
		...
		head.setBrush("bust_manticore_head_0" + this.Math.rand(1, 1));
		head.Color = body.Color;
		head.Saturation = body.Saturation;
		local injury = this.addSprite("injury");
		injury.Visible = false;
		injury.setBrush("bust_manticore_body_01_injured");
		...
		this.m.Skills.add(this.new("scripts/skills/actives/legend_manticore_rake_skill"));
		this.m.Skills.add(this.new("scripts/skills/actives/legend_manticore_rake_zoc_skill"));
		this.m.Skills.add(this.new("scripts/skills/perks/perk_pathfinder"));
		this.m.Skills.add(this.new("scripts/skills/perks/perk_hold_out"));
		this.m.Skills.add(this.new("scripts/skills/perks/perk_executioner"));
		...
		if (this.m.Tail == null)
		{
			...
			if (spawnTile != null)
			{
				this.m.Tail = this.WeakTableRef(this.Tactical.spawnEntity("scripts/entity/tactical/enemies/legend_manticore_tail", spawnTile.Coords.X, spawnTile.Coords.Y, this.getID()));
