this.arena_spectator <- this.inherit("scripts/entity/tactical/entity", {
	...
	if (this.World.LegendsMod.Configs().LegendGenderEnabled() && random >= 50)
		{
			female = 1;
		}
	...
	if (female = 0)
		{
		local body = this.addSprite("body");
			body.setBrush(this.Const.Bodies.SouthernFemale[this.Math.rand(0, this.Const.Bodies.SouthernFemale.len() - 1)]);
			body.varyColor(0.05, 0.05, 0.05);
			body.varySaturation(0.1);
			local armor = this.addSprite("armor");
			...
			local head = this.addSprite("head");
			head.setBrush(this.Const.Faces.SouthernFemale[this.Math.rand(0, this.Const.Faces.SouthernFemale.len() - 1)]);
			head.Color = body.Color;
			head.Saturation = body.Saturation;
			...
			{
				hair.setBrush("hair_brown_" + this.Const.Hair.SouthernFemale[this.Math.rand(0, this.Const.Hair.SouthernFemale.len() - 1)]);
				hair.Saturation = 0.8;
				hair.setBrightness(0.4);
			}
		}
