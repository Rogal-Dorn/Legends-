...
gt.Const.Perks.PerksVala <- [
	...
	[
		...
		},
		{
			...
			Name = this.Const.Strings.PerkName.vazl_ValaInscribeShield,
			Tooltip = this.Const.Strings.PerkDescription.vazl_ValaInscribeShield,
...
gt.Const.Perks.LookupMapVala <- {};
for( local row = 0; row < this.Const.Perks.PerksVala.len(); row = ++row )
{
	for( local i = 0; i < this.Const.Perks.PerksVala[row].len(); i = ++i )
	{
		local perk = this.Const.Perks.PerksVala[row][i];
		perk.Row <- row;
		perk.Unlocks <- row;
		gt.Const.Perks.LookupMapVala[perk.ID] <- perk;
	}
}

gt.Const.Perks.LookupMapInventor <- {};
for( local row = 0; row < this.Const.Perks.PerksInventor.len(); row = ++row )
{
	for( local i = 0; i < this.Const.Perks.PerksInventor[row].len(); i = ++i )
	{
		local perk = this.Const.Perks.PerksInventor[row][i];
		perk.Row <- row;
		perk.Unlocks <- row;
		gt.Const.Perks.LookupMapInventor[perk.ID] <- perk;
	}
}
