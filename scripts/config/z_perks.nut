...
for( local row = 0; row < this.Const.Perks.PerksTreeBeggar.len(); row = ++row )
{
	for( local i = 0; i < this.Const.Perks.PerksTreeBeggar[row].len(); i = ++i )
	{
		local perk = this.Const.Perks.PerksTreeBeggar[row][i];
		perk.Row <- row;
		perk.Unlocks <- row;
