...
gt.Const.Perks.PerksTreeHammer <- [];
gt.Const.Perks.TreeHammerLookupMap <- {};

for( local row = 0; row < HammerTree.len(); row = ++row )
{
	local rowL = [];
	for( local i = 0; i < HammerTree[row].len(); i = ++i )
	{
		local perk = clone this.Const.Perks.PerkDefObjects[HammerTree[row][i]];
		perk.Row <- row;
		perk.Unlocks <- row;
		rowL.push(perk);
		gt.Const.Perks.TreeHammerLookupMap[perk.ID] <- perk;
	}
	gt.Const.Perks.PerksTreeHammer.push(rowL);
}
