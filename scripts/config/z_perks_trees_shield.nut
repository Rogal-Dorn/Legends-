...
{
	local rowL = [];
	for( local i = 0; i < ShieldTree[row].len(); i = ++i )
	{
		local perk = clone this.Const.Perks.PerkDefObjects[ShieldTree[row][i]];
		perk.Row <- row;
		perk.Unlocks <- row;
		rowL.push(perk);
		gt.Const.Perks.TreeShieldLookupMap[perk.ID] <- perk;
	}
	gt.Const.Perks.PerksTreeShield.push(rowL);
}
