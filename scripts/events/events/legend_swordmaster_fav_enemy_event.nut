this.legend_swordmaster_fav_enemy_event <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			ID = "Victory",
			Text = "[img]gfx/ui/events/event_96.png[/img]{%chosen% stands victorious over badly wounded %enemyname%. Defeated nobleman barely catching breath. Every gaps of air comes with great pain for him. Bleeding and shaking, he is soon to depart from world of living. Duel was meant to be to the death but  %chosen% does not deliver a final blow. Rest of your men gathers around  winner. Most of company cheering over glorious  %chosen%, but some are more concern about unfinished business that lays on ground in pool of his own blood. %randombrother% speaks up with strong contempt to loser of this duel. %SPEECH_ON%What you waiting for %chosen%? You think this whipster would show you mercy?%SPEECH_OFF%Company champion answers. %SPEECH_ON%I will take his equipment as reward.  Head has no use for me. %SPEECH_OFF% }",
			Image = "",
			List = [],
			Characters = [],
			Options = [
