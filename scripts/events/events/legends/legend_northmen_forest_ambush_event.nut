this.legend_northmen_forest_ambush_event <- this.inherit("scripts/events/event", {
	m = {
		Barbarian = null, //also, raider
		Wildman = null, // man, woman or berserker (no commander)
		MasterArcher = null, //poacher, huntsman or master archer
		Assassin = null, //all types (no commander)
		Rewarditems = 0
	},
	function create()
	{
			...
			Options = [
				{
					...
					function getResult( _event )
					{
						...
						if (r <= 65)
						{
							return "Lightpatrol"; //light patrol
						}
						else if (r <= 35)
						{
							return "Heavypatrol"; //heavy patrol
						}
					}
		...
		this.m.Screens.push({ //—
			ID = "Wildman", 
			Text = "[img]gfx/ui/events/event_86.png[/img]%wildman% surges forward like a dog unshackled and runs toward the noise. A forward scout crests the treeline and immedtely recieves a rock to the temple for their efforts. %wildman% begins to scream, or rather, continue to scream but louder as he beats the scout to death with a tree bough.\n\n A handful of warriors come over the crest in an attempt to figure out what kind of animal was dying to make such a noise. The first has their jaw broken by the now splintered tree bough while the second is wrestled to the floor by %wildman% and has their eyes gouged for good measure.\n\n The rest of %companyname% do not know wether to cheer on run but cointinue to watch in awe at the frenzied monster clubbing the third northman to death with a sharp rock.\n\n The rest of the patrol finally assemble and %wildman% regains their wits and beats a hasty retreat back to your formation. On of their victims was much better armoured than the rest and you suspect %wildman% just accidently killed their leader before the battle has even begun.\n The patrol\'s numbers have been thinned and your wildling has only a few scratches to show for it!",
			Image = "",
			List = [],
			Characters = [],
			Options = [
