this.rookie_gets_hurt_event <- this.inherit("scripts/events/event", {
	...
	function onUpdateScore()
	{
		...
		{
			if (bro.getLevel() == 1 && !bro.getBackground().getID() == "background.slave" && !bro.getBackground().getID() == "background.slave" && !bro.getBackground().isCombatBackground() && bro.getPlaceInFormation() <= 17 && bro.getLifetimeStats().Battles >= 1)
			{
				candidates.push(bro);
			}
		}
