this.youngling_alp_event <- this.inherit("scripts/events/event", {
	...
	function create()
	{
		...
		this.m.Screens.push({
			ID = "E",
			Text = "[img]gfx/ui/events/event_33.png[/img]{%flagellant% the flagellant is at the rim of the camp self whipped raw. The soul cleansing tool is rigged with broken glass and cat claws, bound tight with leather rinsed taut in urine, and tassels of twisted horse hair. The mercenary walks out into the wilderness, hiding their own back with every step.%SPEECH_ON%It\'s not that I fear you, creature in the shadows. It\'s not that I fear you, shadows in my mind. It\'s not that I fear you, mind in my body.%SPEECH_OFF%The mercenary stops walking, but the urgency of the flagellation increases and you can see the flecks of blood winking in the moonlight.%SPEECH_ON%It is that I fear the old gods of which you are not! Of which you are but an insect!%SPEECH_OFF%The alp\'s silhouette shrinks away, shrieks, and then scurries off altogether. The mercenary returns and collapses into the camp. A few of the company are horrified, while others are emboldened by the courage and righteousness.}",
			Image = "",
			List = [],
			Characters = [],
			Options = [
