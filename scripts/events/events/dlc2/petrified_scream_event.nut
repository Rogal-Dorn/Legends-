		...
		this.m.Screens.push({
			...
			function start( _event )
			{
				...
				foreach( bro in brothers )
				{
					...
					if (bro.getSkills().hasSkill("trait.superstitious") || bro.getSkills().hasSkill("trait.paranoid") || bro.getSkills().hasSkill("trait.dastard") || bro.getSkills().hasSkill("trait.craven") || this.Math.rand(1, 100) <= 33)
					{
