this.undead_necrosavant_event <- this.inherit("scripts/events/event", {
	...
	this.m.Screens.push({
			...
			function start( _event )
			{
				...
				_event.m.getSkills().add(this.new("scripts/skills/racial/skeleton_racial"));
				_event.m.getSkills().getSkills().add(this.new("scripts/skills/injury_permanent/legend_fleshless"));
