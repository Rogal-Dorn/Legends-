this.vazl_inventor_prosthetic_eye <- this.inherit("scripts/events/event", {
	m = {
		Inventor = null,
		Noeye = null
	},
	function create()
	{
		...
		this.m.Screens.push({
			...
			function start( _event )
			{
				...
				local glass_eye_trait = this.new("scripts/skills/traits/vazl_prosthetic_eye");
	...
	function onUpdateScore()
	{
		...
		foreach (bro in brothers)
		{
			if (bro.getSkills().hasSkill("injury.missing_eye") && !bro.getSkills().hasSkill("trait.vazl_prosthetic_eye") && bro.getID() != this.m.Inventor.getID())
			{
