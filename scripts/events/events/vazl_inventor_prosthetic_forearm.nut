this.vazl_inventor_prosthetic_forearm <- this.inherit("scripts/events/event", {
	m = {
		Inventor = null,
		Nofoot = null
	},
	function create()
	{
		this.m.ID = "event.vazl_inventor_prosthetic_forearm";
		this.m.Title = "During camp...";
		this.m.Cooldown = 60 * this.World.getTime().SecondsPerDay;
		this.m.Screens.push({
			ID = "A",
			Text = "[img]gfx/ui/events/vazl_inventor_general.png[/img]%inventor% offers to fix %nofoot%\'s arm. He asks for coin to cover the cost, and requires the use of company tools.",
			Image = "",
			List = [],
			Characters = [],
			Options = [
		...
		this.m.Screens.push({
			ID = "B",
			Text = "[img]gfx/ui/events/vazl_inventor_general.png[/img]%inventor% amputates %nofoot%\'s forearm and attaches an artificial one.",
			Image = "",
			List = [],
			Characters = [],
			Options = [
			...
			function start( _event )
			{
				...
				local pros_foot_trait = this.new("scripts/skills/traits/vazl_prosthetic_forearm");
	...
	{
		this.logInfo("Inventor creates a prosthetic forearm  --  onUpdateScore");
		...
		foreach (bro in brothers)
		{
			if (bro.getSkills().hasSkill("injury.broken_elbow_joint") && !bro.getSkills().hasSkill("trait.vazl_prosthetic_forearm") && bro.getID() != this.m.Inventor.getID())
			{
