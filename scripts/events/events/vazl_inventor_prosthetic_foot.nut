this.vazl_inventor_prosthetic_foot <- this.inherit("scripts/events/event", {
	m = {
		Inventor = null,
		Nofoot = null
	},
	function create()
	{
		...
		this.m.Screens.push({
			ID = "B",
			Text = "[img]gfx/ui/events/vazl_inventor_general.png[/img]%inventor% amputates %nofoot%\'s foot and attaches an artificial one.",
			Image = "",
			List = [],
			Characters = [],
			Options = [
			...
			function start( _event )
			{
				...
				local pros_foot_trait = this.new("scripts/skills/traits/vazl_prosthetic_foot");
	...
	function onUpdateScore()
	{
		...
		foreach (bro in brothers)
		{
			if (bro.getSkills().hasSkill("injury.maimed_foot") && !bro.getSkills().hasSkill("trait.vazl_prosthetic_foot") && bro.getID() != this.m.Inventor.getID())
			{
