this.player_base_faction <- this.inherit("scripts/factions/faction", {
	...
	function update( _ignoreDelay = false, _isNewCampaign = false )
	{
		...
		//ghetto way of implementing alliances for previous users
		...
		local player_base = this.m.Settlements[0]
		...
		//local actionToFire = this.m.Deck[0]
		//actionToFire.execute(this);
	}
	...
	{
		local result = "banner_" + this.m.Banner;
		return result;
	}
	...
	{
		return "ui/banners/banner_" + this.m.Banner + ".png";
	}

	function getUIBannerSmall()
	{
		return "ui/banners/banner_" + this.m.Banner + "s.png";
	}
