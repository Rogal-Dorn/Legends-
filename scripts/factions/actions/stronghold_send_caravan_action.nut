this.stronghold_send_caravan_action <- this.inherit("scripts/factions/faction_action", {
	...
	function onExecute( _faction )
	{
		...
		{
			if (settlement.getID() != player_base.getID() &&  player_base.isConnectedToByRoads(settlement) && (settlement.getOwner() == null || settlement.getOwner().isAlliedWith(player_faction.getID())))
			{
				if (!closest)
				{
		...
		local unload = this.new("scripts/ai/world/orders/mod_unload_order");
