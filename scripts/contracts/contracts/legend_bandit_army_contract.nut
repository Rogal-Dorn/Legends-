this.legend_bandit_army_contract <- this.inherit("scripts/contracts/contract", {
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_04.png[/img]{You return to %townname% to meet with %employer%. Claiming your reward for all the troubles your company had to endure is your main priority. While you are being summoned to %employer% your mercenaries are already trading away spoils of war acquired through this adventure. You head off to the meeting, accompanied by personal guards of %employer% and local militia captains. Everyone present at meeting with %employer% can clearly sense mixed feelings coming from him. On one side relief with the threat eliminated, but on the other anger due to giving away the small fortune that was promised. Finally you are given the reward. Walking around %townname% you notice it has been well prepared for eventuality of siege. This adventure did not come cheap. }",
			Image = "",
			Characters = [],
			List = [],
			ShowEmployer = true,
