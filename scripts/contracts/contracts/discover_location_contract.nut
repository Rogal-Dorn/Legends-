this.discover_location_contract <- this.inherit("scripts/contracts/contract", {
	...
	function setup()
	{
		...
		this.m.Payment.Pool = this.Math.max(100, 10 + lowestDistance * 15.0 * this.getPaymentMult() * this.Math.pow(this.getDifficultyMult(), this.Const.World.Assets.ContractRewardPOW) * this.getReputationToPaymentLightMult());
			...
			Options = [
				{
					...
					function getResult()
					{
						...
						this.Contract.addUnitsToCombat(p.Entities, this.Const.World.Spawn.BanditRaiders, 100 * this.Contract.getDifficultyMult() * this.Contract.getReputationToDifficultyMult(), this.World.FactionManager.getFactionOfType(this.Const.FactionType.Bandits).getID());
