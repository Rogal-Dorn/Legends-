this.discover_location_contract <- this.inherit("scripts/contracts/contract", {
	...
	function setup()
	{
		...
		this.m.Payment.Pool = this.Math.max(300, 100 + (::Legends.Mod.ModSettings.getSetting("ExplorationMode").getValue() ? 100 : 0) + lowestDistance * 15.0 * this.getPaymentMult() * this.Math.pow(this.getDifficultyMult(), this.Const.World.Assets.ContractRewardPOW) * this.getReputationToPaymentLightMult());
