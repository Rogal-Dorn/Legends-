this.discover_location_contract <- this.inherit("scripts/contracts/contract", {
	...
	function setup()
	{
		...
		this.m.Payment.Pool = this.Math.max(10, 10 + lowestDistance * 15.0 * this.getPaymentMult() * this.Math.pow(this.getDifficultyMult(), this.Const.World.Assets.ContractRewardPOW) * this.getReputationToPaymentLightMult());
