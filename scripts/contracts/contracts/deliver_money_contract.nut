this.deliver_money_contract <- this.inherit("scripts/contracts/contract", {
	...
	function createStates()
	{
		...
		this.m.States.push({
			...
			function update()
			{
				...
				{
					local contractValue = (this.Contract.m.Payment.getOnCompletion() + this.Contract.m.Payment.getInAdvance()) * 2;
					if (this.World.Assets.m.Money < contractValue)
					{
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_98.png[/img]{You arrive at the destination, but without the coins. %SPEECH_ON%Where is the coin?%SPEECH_OFF% they ask, but you shrug and say you spent it already. %SPEECH_ON%Then you can only be here for one thing%SPEECH_OFF% }  ",
			Image = "",
			List = [],
			Options = [
				{
					Text = "Thats right, we want the rest.",
					function getResult()
					{
						local p = this.World.State.getLocalCombatProperties(this.World.State.getPlayer().getPos());
						p.CombatID = "Mercs";
						...
						p.PlayerDeploymentType = this.Const.Tactical.DeploymentType.Line;
						p.EnemyDeploymentType = this.Const.Tactical.DeploymentType.Line;
						this.Const.World.Common.addUnitsToCombat(p.Entities, this.Const.World.Spawn.Mercenaries, 200 * this.Contract.getDifficultyMult() * this.Contract.getReputationToDifficultyMult(), this.World.FactionManager.getFactionOfType(this.Const.FactionType.Bandits).getID());
						this.World.Contracts.startScriptedCombat(p, false, true, true);
						local recipientFaction = this.Contract.m.Destination.getFactionOfType(this.Const.FactionType.Settlement);

						if (recipientFaction != null)
						{
							recipientFaction.addPlayerRelation(this.Const.World.Assets.RelationCivilianContractFail * 2, "Killed the contract recipient");
						}
