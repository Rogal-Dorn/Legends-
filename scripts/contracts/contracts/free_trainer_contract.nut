this.free_trainer_contract <- this.inherit("scripts/contracts/contract", {
	//unlock turbo training
	// the more contracts I write, the uglier they get
	m = {
		Reward = 0,
		Title = "Find the legendary trainer of the barbarians",
		LastCombatTime = 0.0,
		Destination = null,
		Chosen = [],
		ChosenOptions = []
	...
	function create()
	{
		...
		this.createStates();
		this.createScreens();
		this.m.Type = "contract.free_trainer_contract";
	...
	{	
			...
			Options = [
				{
					...
					function getResult()
					{
						...
						if (tile != null)
						{
							camp = this.World.spawnLocation("scripts/entity/world/locations/legendary/trainer_location", tile.Coords);
						}
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_167.png[/img]{From a distance, you can already hear the sounds of banging metal and rancious laughter. You don't dare approach without at least three men.}",
			Image = "",
			List = [],
			Options = [
				{
				Text = "We will return another day.",
					function getResult()
					{
						return 0;
					}
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_53.png[/img]{Your champions cut down the Lord of War and his Chosen. Nobody complains as you gather up their gear. The old swordmaster is impressed by the performance of your men. 'So there are some true warriors left among the men of the south. Very well, I will accompany you.' You leave the Fields of War with the swordmaster among you.}",
			Image = "",
			List = [],
			Options = [
				{
		...
		this.m.Screens.push({
			ID = "IsFailure",
			Title = "After the battle...",
			Text = "[img]gfx/ui/events/event_53.png[/img]{Under thunderous cheers by the northmen, your champions are cut down by the Lord of War and his chosen. The old swordmaster sneers: he knew this would happen. You leave the Fields of War, having achieved nothing but losing some of your best men.}",
			Image = "",
			List = [],
			Options = [
				{
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_53.png[/img]{The old swordmaster appreciates your stronghold. He agrees to teach your new men, though his services don't come cheap.}",
			Image = "",
			Characters = [],
			List = [],
			ShowEmployer = true,
