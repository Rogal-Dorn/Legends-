this.find_waterskin_recipe_contract <- this.inherit("scripts/contracts/contract", {
	...
	function create()
	{
		...
		this.createStates();
		this.createScreens();
		this.m.Type = "contract.find_waterskin_recipe_contract";
		...
		this.m.States.push({
			...
			function start()
			{
				...
				else
				{
					...
					if (tile != null)
					{
						camp = this.World.spawnLocation("scripts/entity/world/locations/legendary/waterskin_location", tile.Coords);
					}
	...
	{	
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_161.png[/img]{After defeating the defenders of the camp, you search the remains. You find an ancient parchment, filled with an unrecognisable script. You also find a flask, filled with a strange liquid. You suspect that this is the fabled water of life. The poor priests couldn't drink it, as they were skeletons.}",
			Image = "",
			List = [],
			Options = [
				{
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_167.png[/img]{You find the remains of a desert camp. It is surrounded by a carpet of human bones. Some of the bones in the center start to move, and a small group of skeletons assemble themselves, led by three priests. As you wonder how this pitiful group has managed to survive this long, the ground begins to rumble, and a host of massive, enchanted rocks begin to emerge.}",
			Image = "",
			List = [],
			Options = [
				{
