this.free_mercenaries_contract <- this.inherit("scripts/contracts/contract", {
	...
	function create()
	{
		...
		this.createStates();
		this.createScreens();
		this.m.Type = "contract.free_mercenaries_contract";
	...
	{	
		this.m.Screens.push({
			ID = "Task",
			Title = this.m.Title,
			Text = "A band of mercenaries is being transported to a fortress and is awaiting execution. We must help them!",
			Image = "",
			List = [],
			ShowEmployer = true,
			Options = [
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_53.png[/img]{After defeating the noble army, you release the mercenaries from their chains. They retrieve their gear and follow you home.}"
			Image = "",
			List = [],
			Options = [
				{
		...
		this.m.Screens.push({
			ID = "Failure1",
			Title = "On your return...",
			Text = "[img]gfx/ui/events/event_95.png[/img]{You couldn't save the mercenaries in time. They were executed a day later. This one's on you!}"
			Image = "",
			Characters = [],
			List = [],
			ShowEmployer = true,
