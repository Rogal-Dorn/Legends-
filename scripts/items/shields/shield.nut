this.shield <- this.inherit("scripts/items/item", {
	...
	function isDroppedAsLoot()
	{
		...
		if ((isBlacksmithed || this.m.Condition >= 6) && (isPlayer || this.m.Condition / this.m.ConditionMax >= 0.25) && (isPlayer || isLucky || isBlacksmithed || this.Math.rand(1, 100) <= 90))
		{
			return true;
		}
	...
	function applyShieldDamage( _damage, _playHitSound = true )
	{
		...
		if (Condition == 0)
		{
			...
			local isBlacksmithed = this.isNamed() || (isPlayer && !this.Tactical.State.isScenarioMode() && this.World.Assets.m.IsBlacksmithed);
